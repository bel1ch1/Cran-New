...
| Раздел | Описание |
|--------|----------|
| [**Развёртывание**](docs/deployment.md) | Полный гайд: Docker, камеры, калибровка, обновление |
| [**Методологии**](docs/methodology/README.md) | Алгоритмы XY/Hooks, Modbus, фильтрация pose, архитектура |

## Развёртывание

Краткий путь для уже подготовленного хоста:

```bash
git clone <URL> /home/cran/Cran-New && cd /home/cran/Cran-New
cp data/calibration_config.example.json data/calibration_config.json
cp .env.example .env   # смените пароль и CRAN_SESSION_SECRET
docker compose up -d --build
docker compose ps      # 5 сервисов: app, bridge, hook, influxdb, influx-writer
```

Web UI: `http://<IP>:8000` · Modbus: `:5020` · InfluxDB: `:8086`

**Подробно:** камеры, `.env`, калибровка, troubleshooting → [docs/deployment.md](docs/deployment.md).
...
*(Дублирует [docs/deployment.md](docs/deployment.md) с акцентом на первичную настройку ОС.)*
...
# Pose runtime (мост) — см. docs/methodology/pose-filtering-and-tuning.md
CRAN_POSE_FPS=10
...
# Spatial map (калибровка + runtime match)
CRAN_MIN_TRUST_HITS=7
CRAN_MAX_TRUST_SIGMA_M=0.05
CRAN_MIN_LANDMARK_SEPARATION_M=0.03
CRAN_MERGE_TOLERANCE_M=0.02
CRAN_RUNTIME_MATCH_TOLERANCE_M=0.04

# InfluxDB (история для /statistics)
CRAN_INFLUX_PUBLISHED_PORT=8086
CRAN_INFLUX_ORG=cran
CRAN_INFLUX_BUCKET=cran
CRAN_INFLUX_TOKEN=cran-influx-token-change-me
CRAN_INFLUX_MEASUREMENT=crane_pose
CRAN_INFLUX_WRITE_INTERVAL=1.0
EOF
```
...
Ожидаются **пять** контейнеров в статусе `running` / `healthy`:
...
| `cran_influxdb` | База истории pose |
| `cran_pose_influx_writer` | Запись Modbus → InfluxDB |
...
Подробнее о логике XY-калибровки — [docs/methodology/xy-spatial-calibration.md](docs/methodology/xy-spatial-calibration.md).
...
- `app/services/bridge_pose_estimator.py` - runtime pose моста (spatial match + fusion).
...
Текущая реализация моста строит **карту точек по координатам X** (не по ArUco ID): опорный маркер задаёт ноль, остальные точки подтверждаются системой доверия. Подробнее: [docs/methodology/xy-spatial-calibration.md](docs/methodology/xy-spatial-calibration.md).
...
## Standalone программа #1 (Bridge pose + Modbus server)

Скрипт `bridge_pose_modbus.py` (логика в `app/services/bridge_pose_estimator.py`):

- читает `data/calibration_config.json` (блок `bridge_calibration`);
- использует `roi` для поиска ArUco-маркеров;
- сопоставляет детекции с **landmarks по координате X** через `SpatialMarkerMap`;
- fusion: outlier filter, gate, медианное окно, EMA;
- поднимает Modbus TCP сервер и публикует holding-регистры.

Методология: [docs/methodology/bridge-pose-runtime.md](docs/methodology/bridge-pose-runtime.md) · Modbus: [docs/methodology/modbus-protocol.md](docs/methodology/modbus-protocol.md).
...
- `--fps` частота CLI (переопределяется `CRAN_POSE_FPS` из `.env`);
...
См. `.env.example`, раздел «Доступ» выше и [docs/methodology/pose-filtering-and-tuning.md](docs/methodology/pose-filtering-and-tuning.md).

- `CRAN_CAMERA_BACKEND` — `picamera2`, `v4l2`, `gstreamer`, `jetson`, `auto`
- `CRAN_POSE_*` — фильтрация и частота runtime pose
- `CRAN_MIN_TRUST_HITS`, `CRAN_MAX_TRUST_SIGMA_M`, `CRAN_MERGE_TOLERANCE_M`, … — spatial map
