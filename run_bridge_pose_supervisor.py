...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--restart-delay",
        type=float,
        default=_env_float("CRAN_BRIDGE_RESTART_DELAY", 1.0),
    ...
    parser.add_argument(
        "--heartbeat-file",
        type=Path,
        default=Path("data/runtime/bridge_pose_supervisor.heartbeat"),
...
def main() -> int:
    ...
    child_cmd = [args.python, "bridge_pose_modbus.py", *child_args]
        ...
        while not stop["value"]:
            ...
            if _is_lock_active(args.lock_file):
                if child is not None:
                    print("[SUPERVISOR] calibration lock detected, stopping bridge child...")
                    _terminate_child(child, args.child_pid_file)
                    child = None
                if now - lock_wait_msg_ts >= 5:
                    print("[SUPERVISOR] bridge paused while calibration lock is active")
